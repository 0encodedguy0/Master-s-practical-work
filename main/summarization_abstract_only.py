...
results_df.to_csv("/data/arxiv_summarization_results_abstract.csv", index=False)
...
plt.title("ROUGE-1 Comparison Across Models (abstract only)")
plt.xticks(rotation=45, ha="right")
plt.tight_layout()
plt.savefig("/pics/rouge1_comparison_abstract.png")
plt.show()

# ROUGE-2
plt.figure(figsize=(12,6))
sns.barplot(x="Model", y="ROUGE-2", data=results_df, palette="Greens_d")
plt.title("ROUGE-2 Comparison Across Models (abstract only)")
plt.xticks(rotation=45, ha="right")
plt.tight_layout()
plt.savefig("/pics/rouge2_comparison_abstract.png")
plt.show()

# BLEU
plt.figure(figsize=(12,6))
sns.barplot(x="Model", y="BLEU", data=results_df, palette="Reds_d")
plt.title("BLEU Comparison Across Models (abstract only)")
plt.xticks(rotation=45, ha="right")
plt.tight_layout()
plt.savefig("/pics/bleu_comparison_abstract.png")
plt.show()
...
plt.title("Speed vs Quality Tradeoff (abstract only)")
plt.xlabel("Average Time per Sample (sec)")
plt.ylabel("ROUGE-1")
plt.tight_layout()
plt.savefig("/pics/speed_vs_quality_abstract.png")
plt.show()
